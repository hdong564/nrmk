...
# acc
    ...
    def get_joint_pos(self):
        shm_val = self.shm_access(INDY_SHM_ROBOT_ADDR_CTRL_Q, 8 * 6, '6d')
        res = []
        for val in shm_val:
    ...
    def get_joint_vel(self):
        return [rad2deg(x) for x in self.shm_access(INDY_SHM_ROBOT_ADDR_CTRL_QDOT, 8*self.joint_dof, '%sd'%self.joint_dof)]

    def get_task_pos(self):
        shm_val = self.shm_access(INDY_SHM_ROBOT_ADDR_CTRL_P, 8 * 6, '6d')
    ...
    #     return [rad2deg(x) for x in self.shm_access(INDY_SHM_ROBOT_ADDR_CTRL_QDDOT, 8*self.joint_dof, '%sd'%self.joint_dof)]
    #
    ...
    def get_last_emergency_info(self):
        ...
        res = {attr[0]: error_code[shm_val[0] - 1],
               attr[1]: shm_val[1:4],
               attr[2]: shm_val[4:7],
               attr[3]: shm_val[7]}
    ...
    def set_prog_ratio(self, ratio):
        if ratio > 1.0:
            ratio = 1.0
        if ratio < 0.0:
            ratio = 0.0

        add_arg = {'cmd_name': INDY_SHM_CLIENT_ADDR64_VAL_PROGRAM_SPD_RATIO,
            'cmd_size': 8,
            'data_type': 'd',
            'data': ratio}
        return self.command_set_logic(INDY_SHM_CLIENT_ADDR8_CMD_SET_PROGRAM_RATIO, add_arg)

    def set_cmd_ratio(self, ratio):
        if ratio > 1.0:
            ratio = 1.0
        if ratio < 0.0:
            ratio = 0.0 

        add_arg = {'cmd_name': INDY_SHM_CLIENT_ADDR64_VAL_COMMAND_SPD_RATIO,
            'cmd_size': 8,
            'data_type': 'd',
            'data': ratio}
        return self.command_set_logic(INDY_SHM_CLIENT_ADDR8_CMD_SET_COMMAND_RATIO, add_arg)