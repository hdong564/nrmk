...
def co_routine(worker: Worker, deltatime: timedelta, logger: Logger):
    ...
    for idx in _lift_usage.keys():
        ...
        if _lift_usage[idx]:
            ...
            elif di_down_state and not cup_exists:       
                if _lift_error_flag[idx] == False:
                    logger.info("errer!! cup is removed while lift down ")
                    _lift_error_flag[idx] = True
                    _lift_usage[idx] = False
            elif not hand_detected and (not cup_exists or is_expired_coffee) and not _lift_error_flag[idx]:
                _lift_usage[idx] = False
            ...
            if not di_up_state:
                if hand_detected:
                    ...
                    shm.set_do(addr_do_lift_down, True)
                    shm.set_do(addr_do_lift_up, False)
                    _lift_error_flag[idx] = False # no error if no hand detection 
                elif not cup_exists or is_expired_coffee:
            ...
            elif _lift_error_flag[idx] == True:
                logger.info(f"[warining] lift is up state and not exisit_cup in logic, but exisit_cup in real.... expire start.... idx : {idx}")
                _lift_error_flag[idx] = False
                is_expired_coffee = True
                #익스파이어 = true
            elif is_coffee_slot and (not cup_exists or is_expired_coffee):
                _lift_delay[idx] = 0
                coffee: Coffee = _lift_slot[idx]
...
def on_barcode_listener(sender, args: BarcodeArgs):
    ...
    for idx in _lift_slot.keys():
        slot = _lift_slot[idx]
        with _context.logger() as logger:
            state = slot is not None and isinstance(slot, Coffee)
            if state:
                logger.info(f"State: {state}, position: {slot.get_position_state().name}")
            else:
                logger.info(f"State: {state}, The slot[{idx}] is None. Skip this index.")
            ...
            if state and slot.get_position_state() == Coffee.Position.LIFT:
                ...
                if barcode.strip() == slot.get_order_data().PE_ID.strip() and not slot.is_expired():
