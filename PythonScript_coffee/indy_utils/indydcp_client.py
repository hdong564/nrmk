...
CMD_SET_VAR_REFFRAME                        = 118 # new 2.4.2
CMD_GET_VAR_REFFRAME                        = 119 # new 2.4.2
CMD_APPLY_VAR_REFFRAME                         = 120 # new 2.4.2
...
CMD_READ_EL1008                             = 900 # 수정필요
CMD_WRITE_EL4104                            = 900 # 수정필요
CMD_WRITE_EL2008                            = 901 # 수정필요
...
def tcp_command(cmd, response_type=None, response_len=None):
    def decorate(func):
        def decorated(*args, **kwargs):
            global JOINT_DOF
            ...
            if response_type == 'jointArr':
                ...
                if response_len is None:
                    ...
                    return np.array(eval('_res_data.' + response_type)).tolist()[:response_len]
            else:
                return None
...
class IndyDCPClient:
    ...
    def sync_pause_cond(self):
        return GLOBAL_DICT['pause']
        # return self.read_direct_variable(10, 1) == 10
        # GLOBAL_DICT['pause'] # 여기에 pause 조건
...
# gwkim temp
    @tcp_command(CMD_SET_VAR_REFFRAME)
    def set_var_reference_frame(self, ref):
        data = Data()
        data_size = 6 * 8
        for i in range(JOINT_DOF):
            data.double6dArr[i] = ref[i]
    ...
    @tcp_command(CMD_GET_VAR_REFFRAME, 'double6dArr')
    def get_var_reference_frame(self):
        pass

    @tcp_command(CMD_APPLY_VAR_REFFRAME)
    def apply_var_reference_frame(self):
        pass
# gwkim temp
...
# gwkim
    ...
    @tcp_command(CMD_WRITE_EL4104)
    def set_el4104_ao(self, arr):
        data = Data()
        data_size = 2 * 4
        data.shortVal4Arr[0] = arr[0]
        data.shortVal4Arr[1] = arr[1]
        data.shortVal4Arr[2] = arr[2]
        data.shortVal4Arr[3] = arr[3]

        return (data, data_size)
       

    def get_el1008_di(self):
        error_code, _res_data, _ = self._handle_command(CMD_READ_EL1008)

        if error_code:
            return error_code
        print(_res_data.byteVal)
        result = [0, 0, 0, 0, 0, 0, 0, 0]
        if _res_data.byteVal < 256:
            b = list("{0:b}".format(_res_data.byteVal))
            for i in range(len(b)):
                result[i] = int(b[len(b)-i-1])
            return result
        else:
            print("out of range :", _res_data.byteVal)
            return None
            
    @tcp_command(CMD_WRITE_EL2008)
    ...
    def set_endtool_do(self, endtool_type, val):
        ...
        # ROBOTIQ
        # endtool_type : 0 control mode setting
        # val : initialization: 16, position control: 32, velocity setting: 48, force senting: 64, data gathering: 80
        # ex) (0, 32) : set mode to position control 
        # endtool_type : 1 control value setting
        # val :  control value can be selected between 0 - 255
        # ex) (0, 32) and (1,255) : full open 
    ...
    @tcp_command(CMD_GCODE_EXECUTE)
    def gcode_execute(self):
        pass

    @tcp_command(CMD_GCODE_RESET)
    def gcode_reset(self):
        pass
    ...
    def move_ext_traj_txt_file(self, file_name):
        ...
        req_ext_data = file_name.encode('ascii')
        req_ext_data_size = len(file_name)
        print(req_ext_data, req_ext_data_size)
